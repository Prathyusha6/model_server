...
TEST(DemultiplexerTest, CheckDemultipliedBlobsMultipleOutputs) {
    ...
    for (size_t shardId = 0; shardId < demultiplyCount; ++shardId) {
        ...
        SPDLOG_INFO("Shard:{}, sessionKey:{}", shardId, sessionResult.first.getSessionKey());
        for (auto& [blobName, blob] : sessionResult.second) {
            SPDLOG_INFO("Shard:{} Checking blobName:{} session_key:{}", shardId, blobName, sessionResult.first.getSessionKey());
